...
const getHomeCMSDataCached = unstable_cache(_getHomeCMSData, ['cms-home'], {
  revalidate: 60 * 60, // 1 hour (invalidated immediately via revalidateTag)
  tags: [CacheTags.cms.home],
});

export async function getHomeCMSData() {
  try {
    return await getHomeCMSDataCached();
  } catch (error) {
