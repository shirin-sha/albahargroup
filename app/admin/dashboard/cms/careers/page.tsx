...
const CAREERS_PAGE_SECTIONS = ['metadata', 'banner', 'careerSection'] as const;

const SECTION_META: Record<string, { label: string; desc: string; icon: string }> = {
  metadata: { label: 'Metadata', desc: 'Careers page SEO title and description', icon: '🔎' },
  banner: { label: 'Banner', desc: 'Careers page banner title', icon: '🖼️' },
  careerSection: { label: 'Career Section', desc: 'Main careers content section', icon: '💼' },
};
...
const BilingualField = memo(({ label, path, type = 'text', rows = 1, placeholder = '', enValue, arValue, onUpdate }: BilingualFieldProps) => {
  if (type === 'textarea') {
    return (
